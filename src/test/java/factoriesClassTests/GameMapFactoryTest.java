...
/**
 * Tests for the MapFactory class and its subclasses
 * 
 * @author Andrea Sessa
 * @author Giorgio Pea
 * @see GameMapFactory
 * @see GameMap
 * @see FermiGameMapFactory
 * @see Sector
 * @see SectorType
 * @see Coordinate
 */
public class GameMapFactoryTest {
	...
	// The fermi map created using fermiMapFactory's make method
	private GameMap fermiMapFromFile = new FermiGameMapFactory().makeMap();
	// The fermi map's graph from the fermiMapFromFile
	private UndirectedGraph<Sector, DefaultEdge> fermiMapGraphFromFile = fermiMapFromFile
			.getGraph();
	// The fermi map's graph created statically by the makeFermiMapStaticGraph
	// method
	...
	private UndirectedGraph<Sector, DefaultEdge> makeFermiMapStaticGraph() {
		...
		Sector s8 = new Sector(new Coordinate('j', 1), SectorType.CLOSED_RESCUE);
		Sector s9 = new Sector(new Coordinate('j', 3), SectorType.DANGEROUS);
		Sector s10 = new Sector(new Coordinate('j', 4), SectorType.SAFE);
		Sector s11 = new Sector(new Coordinate('j', 5),
				SectorType.CLOSED_RESCUE);
	...
	public void connectionsCompositionTest() {
		/*
		 * This test check if all the edges of fermiMapGraphFromFile are present
		 * in fermiMapGraphStatic
		 */
	...
	public void sectorCompositionTest() {
		/*
		 * This test check if all the vertices of fermiMapGraphFromFile are
		 * present in fermiMapGraphStatic. A case in which no vertices are
		 * present is considered as well
		 */
		Iterator<Sector> iterator = fermiMapGraphStatic.vertexSet().iterator();
		while (iterator.hasNext()) {
	...
	public void sizeTest() {
		/*
		 * This test check if the number vertices and edges of
		 * fermiMapGraphFromFile is the same as in fermiMapGraphStatic
		 */
