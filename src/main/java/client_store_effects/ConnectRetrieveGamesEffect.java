 ...
 */
public class ConnectRetrieveGamesEffect implements Effect {
    @Override
    public void apply(StoreAction action, State state) {
        ClientState castedState = (ClientState) state;
        castedState.gamePollingTimer.scheduleAtFixedRate(new GamePollingThread(),0,castedState.gamePollingPeriod);
    }
}
